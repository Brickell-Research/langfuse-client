...
/// source)` in the given window. Only numeric and boolean scores are
/// returned.
pub fn score_value_query(
  from from_timestamp: String,
  to to_timestamp: String,
) -> ScoreValueQuery {
  ScoreValueQuery(from_timestamp:, to_timestamp:)
}

@target(erlang)
/// Aggregate avg score values for the query. One row per `(name,
/// data_type, source)` combination over the window. Erlang-only — see
